...
public class MetaDataRegressionTest extends BaseTestCase {
    ...
    public void testBug11781() throws Exception {
        ...
        createTable("`com.app tab`", "( C1 int(11) NULL, C2 int(11) NULL, INDEX NEWINX (C1), INDEX NEWINX2 (C1, C2))", "InnoDB");

        this.stmt.executeUpdate("ALTER TABLE `com.app tab` ADD CONSTRAINT APPFK FOREIGN KEY (C1) REFERENCES `com.app tab` (C1)");

        /*
         * this.rs = this.conn.getMetaData().getCrossReference(
         * this.conn.getCatalog(), null, "com.app tab", this.conn.getCatalog(),
         * null, "com.app tab");
         */
        this.rs = ((com.mysql.jdbc.DatabaseMetaData) this.conn.getMetaData()).extractForeignKeyFromCreateTable(this.conn.getCatalog(), "com.app tab");
        assertTrue("must return a row", this.rs.next());

        String catalog = this.conn.getCatalog();

        assertEquals("comment; APPFK(`C1`) REFER `" + catalog + "`/ `com.app tab` (`C1`)", this.rs.getString(3));

        this.rs.close();

        this.rs = this.conn.getMetaData().getImportedKeys(this.conn.getCatalog(), null, "com.app tab");

        assertTrue(this.rs.next());

        this.rs = this.conn.getMetaData().getExportedKeys(this.conn.getCatalog(), null, "com.app tab");
